...
架构：
  ...
  Xavier 负责: 视频采集、TensorRT推理/PyTorch原生推理、后处理解码、身体朝向估算、
               多目标优先级选择、压枪补偿、准星校准
...
@dataclass
class ModelConfig:
    """模型配置 (支持 TensorRT / PyTorch 双后端)"""
    # 推理后端选择: "pytorch" | "tensorrt"
    # "pytorch" - 使用官方 .pth 权重文件 (MMPose 原生推理, 默认)
    # "tensorrt" - 使用 TensorRT .trt 引擎文件 (需预先转换)
    inference_backend: str = "pytorch"

    # TensorRT 引擎路径 (inference_backend="tensorrt" 时使用)
    engine_path: str = "models/rtmo_l_640x640.trt"
    onnx_path: str = "models/rtmo_l_640x640.onnx"

    # PyTorch 原生推理路径 (inference_backend="pytorch" 时使用)
    # MMPose 配置文件 (从官方仓库下载)
    pytorch_config_path: str = "configs/rtmo-l_16xb16-600e_coco-640x640.py"
    # PyTorch 权重文件 (.pth) (从 MMPose Model Zoo 下载)
    pytorch_checkpoint_path: str = "models/rtmo-l_16xb16-600e_coco-640x640-6b10eda5_20231211.pth"
    # PyTorch 推理设备
    pytorch_device: str = "cuda:0"
    # 是否启用 FP16 半精度推理 (PyTorch)
    pytorch_fp16: bool = True

    # 模型输入尺寸
    input_width: int = 640
    input_height: int = 640

    # 预处理参数 (MMPose 标准预处理, BGR->RGB 归一化)
