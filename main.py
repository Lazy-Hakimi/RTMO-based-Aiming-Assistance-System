...
3. 准星校准系统 (自动/手动)
4. ESP32-S3 HID鼠标桥接
5. PyTorch 原生推理引擎 (支持 .pth 权重文件, FP16)
6. 双后端推理: PyTorch (默认) / TensorRT
...
推理后端切换:
  - PyTorch (默认): 使用官方 .pth 权重, MMPose 原生推理, 支持 FP16
  - TensorRT: 使用预转换的 .trt 引擎 (需先运行 onnx2trt.py 转换)
...
try:
    ...
    logger.warning("PyTorch 推理引擎不可用 (mmpose 未安装)")
...
def create_inference_engine(backend: str = None):
    """
    创建推理引擎 (支持 PyTorch / TensorRT 双后端)
    
    Args:
        backend: 推理后端 "pytorch" | "tensorrt" | None(从配置读取)
        
    Returns:
        推理引擎实例 (PytorchInferenceEngine 或 TrtInferenceEngine)
    """
    if backend is None:
        backend = MODEL_CFG.inference_backend

    if backend == "pytorch":
        # ===== PyTorch 原生推理 (默认) =====
        if not HAS_PYTORCH_ENGINE:
            logger.error("=" * 60)
            logger.error("PyTorch 推理引擎不可用!")
            logger.error("缺少依赖: mmpose / mmcv / mmengine / torch")
            logger.error("")
            logger.error("安装步骤:")
            logger.error("  1. 安装 PyTorch (Jetson 版本):")
            logger.error("     wget https://developer.download.nvidia.com/"
                         "compute/redist/jp/v512/pytorch/torch-2.1.0a0+"
                         "41361538.nv23.06-cp38-cp38-linux_aarch64.whl")
            logger.error("     pip3 install torch-*.whl")
            logger.error("")
            logger.error("  2. 安装 MMCV (Jetson 版本):")
            logger.error("     pip install mmcv==2.1.0 -f "
                         "https://download.openmmlab.com/mmcv/dist/"
                         "cu118/aarch64/index.html")
            logger.error("")
            logger.error("  3. 安装 MMPose:")
            logger.error("     pip install mmpose>=1.3.0 mmengine")
            logger.error("")
            logger.error("  4. 下载 RTMO-l 配置文件和权重:")
            logger.error("     mkdir -p configs models")
            logger.error("     # 从 MMPose 官方仓库复制配置文件")
            logger.error("     # 从 MMPose Model Zoo 下载 .pth 权重")
            logger.error("=" * 60)
            raise RuntimeError(
                "PyTorch 推理引擎依赖缺失，请按上述步骤安装。\n"
                "或使用 TensorRT 后端: --backend tensorrt"
            )

        logger.info("=" * 60)
        logger.info("使用 PyTorch 原生推理引擎 (FP16)")
        logger.info("=" * 60)
        engine = PytorchInferenceEngine(
            config_path=MODEL_CFG.pytorch_config_path,
            checkpoint_path=MODEL_CFG.pytorch_checkpoint_path,
            device=MODEL_CFG.pytorch_device,
            fp16=MODEL_CFG.pytorch_fp16,
        ...
        # ===== TensorRT 推理 =====
        logger.info("=" * 60)
        logger.info("使用 TensorRT 推理引擎")
        logger.info("=" * 60)
        engine = TrtInferenceEngine(MODEL_CFG.engine_path)
        return engine

    else:
        raise ValueError(f"不支持的推理后端: {backend}. "
                         f"请使用 'pytorch' 或 'tensorrt'")


class AimBotPipelineEnhanced:
    """增强版多线程流水线控制器"""

    def __init__(self, backend: str = None, engine_path: str = None,
                 dummy_mouse: bool = False, dummy_esp32: bool = False):
        logger.info("=" * 60)
        logger.info("RTMO AimBot Enhanced 多线程流水线初始化")
        logger.info("=" * 60)

        self.backend = backend or MODEL_CFG.inference_backend
        ...
        # 1. 加载推理引擎 (PyTorch / TensorRT 自动选择)
        logger.info(f"加载推理引擎: {self.backend}")
        if engine_path and self.backend == "tensorrt":
            # 使用命令行指定的 TensorRT 引擎路径
            MODEL_CFG.engine_path = engine_path
        self.engine = create_inference_engine(self.backend)
    ...
    def _infer_loop(self):
        """推理线程 (增强版)"""
        logger.info("推理线程启动")
        logger.info(f"推理后端: {self.backend}")
...
def main():
    ...
    parser.add_argument("--backend", type=str,
                        default=MODEL_CFG.inference_backend,
                        choices=["pytorch", "tensorrt"],
                        help="推理后端: pytorch (默认, 使用 .pth) | tensorrt (使用 .trt)")
    # TensorRT 引擎路径 (兼容旧参数)
    parser.add_argument("--engine", type=str, default=MODEL_CFG.engine_path,
                       help="TensorRT 引擎路径 (--backend tensorrt 时使用)")
    # PyTorch 模型路径
    parser.add_argument("--pytorch-config", type=str,
                       default=MODEL_CFG.pytorch_config_path,
                       help="MMPose 配置文件路径 (--backend pytorch 时使用)")
    parser.add_argument("--pytorch-checkpoint", type=str,
                       default=MODEL_CFG.pytorch_checkpoint_path,
                       help="PyTorch 权重文件路径 (--backend pytorch 时使用)")
    # 其他参数
    ...
    # 推理后端
    MODEL_CFG.inference_backend = args.backend

    # PyTorch 路径
    MODEL_CFG.pytorch_config_path = args.pytorch_config
    MODEL_CFG.pytorch_checkpoint_path = args.pytorch_checkpoint
